...
namespace mxlib {
    ...
    template <typename T, typename = std::enable_if_t<std::is_arithmetic_v<T>>> struct spec {
    ...
    MXLIB_FUNC_DECL static T reinterpret(U&& src_) {
        constexpr auto valid_types = std::is_pointer_v<T> == std::is_pointer_v<U>;
    ...
    // vector
    ///////////////////////
    template <typename T, uint32_t D, typename = std::enable_if_t<std::is_arithmetic_v<T>>>
    struct vector_t {
        using value_type = T;
        ...
        template <typename... ARGS> vector_t<T, sizeof...(ARGS)> swizzle(ARGS&&... idxs) const {
        ...
        vector_t operator*(T scalar_) const;

        vector_t operator/(T scalar_) const;

        vector_t operator+(vector_t other_) const;
    ...
    struct matrix_t {
        ...
        explicit matrix_t(T* array_) { std::memcpy(data(), array_, sizeof(T) * D0 * D1); }
    ...
    // quaternion
    ///////////////////////
    template <typename T, typename = std::enable_if_t<std::is_floating_point_v<T>>> struct xquat {
        xquat() = default;
        ...
        explicit xquat(T* array_) { std::memcpy(data(), array_, sizeof(T) * 4); }

        T& operator[](uint32_t i);

        const T& operator[](uint32_t i) const;

        xquat operator*(T scalar_) const;
    ...
    template <typename T> using xmtx4x4 = matrix_t<T, 4, 4>;
    ...
    template <typename T> using xvec4 = vector_t<T, 4>;
    template <typename T> using xvec3 = vector_t<T, 3>;
    template <typename T> using xvec2 = vector_t<T, 2>;
    ...
    template <typename T> MXLIB_FUNC_DECL static xmtx4x4<T> matrix_identity();

    template <typename T>
    MXLIB_FUNC_DECL static xmtx4x4<T> matrix_translation(const xvec3<T>& position_);

    template <typename T> MXLIB_FUNC_DECL static xmtx4x4<T> matrix_scale(const xvec3<T>& scale_);

    template <typename T> MXLIB_FUNC_DECL static xmtx4x4<T> matrix_rotation(const xquat<T>& rot_);

    template <typename T>
    MXLIB_FUNC_DECL static xmtx4x4<T> matrix_multiply(xmtx4x4<T> a, xmtx4x4<T> b);

    template <typename T>
    MXLIB_FUNC_DECL static xmtx4x4<T> compose_model_matrix(const xvec3<T>& position_,
                                                           const xquat<T>& rotation_,
                                                           const xvec3<T>& scale_);

    template <typename T> MXLIB_FUNC_DECL static xquat<T> to_quat(const xvec3<T>& euler_);

    template <typename T> MXLIB_FUNC_DECL static xvec3<T> to_euler(const xquat<T>& quat_);

    template <typename T>
    MXLIB_FUNC_DECL static xvec3<T> transform(const xvec3<T>& vec_, const xmtx4x4<T>& mtx_);

    template <typename T> MXLIB_FUNC_DECL static T sq_magnitude(const xquat<T>& quat_);

    template <typename T> MXLIB_FUNC_DECL static xquat<T> conjugate(const xquat<T>& quat_);

    template <typename T> MXLIB_FUNC_DECL static xquat<T> inverse(const xquat<T>& quat_);

    template <typename T> MXLIB_FUNC_DECL static xmtx4x4<T> inverse(const xmtx4x4<T>& mtx_);

    template <typename T> MXLIB_FUNC_DECL static xvec3<T> negate(const xvec3<T>& v0);

    template <typename T>
    MXLIB_FUNC_DECL static xvec3<T> multiply(const xvec3<T>& v0, float scalar);

    template <typename T> MXLIB_FUNC_DECL static xvec3<T> divide(const xvec3<T>& v0, float scalar);
    ...
    template <typename T>
    MXLIB_FUNC_DECL static T mixed_product(const xvec3<T>& v0, const xvec3<T>& v1,
                                           const xvec3<T>& v2);

    template <typename T>
    MXLIB_FUNC_DECL static xvec3<T> triple_product(const xvec3<T>& v0, const xvec3<T>& v1,
                                                   const xvec3<T>& v2);

    template <typename T> MXLIB_FUNC_DECL static T dist_sq(const xvec3<T>& a, const xvec3<T>& b);

    template <typename T> MXLIB_FUNC_DECL static T dist(const xvec3<T>& a, const xvec3<T>& b);

    template <typename T> MXLIB_FUNC_DECL static T magnitude_sq(const xvec3<T>& v);

    template <typename T> MXLIB_FUNC_DECL static T magnitude(const xvec3<T>& v);

    template <typename T>
    MXLIB_FUNC_DECL static xvec3<T> lerp(const xvec3<T>& a, const xvec3<T>& b, float t);

    template <typename T>
    MXLIB_FUNC_DECL static float invlerp(const xvec3<T>& a, const xvec3<T>& b, const xvec3<T>& v);
} // namespace mxlib

namespace mxlib {
    template <typename T, uint32_t N> struct fixed_list {
        using value_type = T;
...
namespace mxlib {
    ...
    template <typename T, uint32_t D, typename _Enable>
    T& vector_t<T, D, _Enable>::operator[](uint32_t i) {
        return _v[i];
    }

    template <typename T, uint32_t D, typename _Enable>
    const T& vector_t<T, D, _Enable>::operator[](uint32_t i) const {
        return _v[i];
    }

    template <typename T, uint32_t D, typename _Enable>
    vector_t<T, D, _Enable> vector_t<T, D, _Enable>::operator*(T scalar) const {
        return {_v[0] * scalar, _v[1] * scalar, _v[2] * scalar};
    }
    ...
    template <typename T, uint32_t D, typename _Enable>
    vector_t<T, D, _Enable> vector_t<T, D, _Enable>::operator+(vector_t other) const {
    ...
    template <typename T, uint32_t D0, uint32_t D1, typename _Enable>
    T& matrix_t<T, D0, D1, _Enable>::operator[](uint32_t i) {
        return _m[i];
    }

    template <typename T, uint32_t D0, uint32_t D1, typename _Enable>
    const T& matrix_t<T, D0, D1, _Enable>::operator[](uint32_t i) const {
        return _m[i];
    }

    template <typename T, typename _Enable> T& xquat<T, _Enable>::operator[](uint32_t i) {
        return _q[i];
    }

    template <typename T, typename _Enable>
    const T& xquat<T, _Enable>::operator[](uint32_t i) const {
        return _q[i];
    }

    template <typename T, typename _Enable>
    xquat<T, _Enable> xquat<T, _Enable>::operator*(T scalar) const {
        return {_q[0] * scalar, _q[1] * scalar, _q[2] * scalar, _q[3] * scalar};
    }
    ...
    template <typename T, typename> bool any(T bitmask) { return bitmask > 0; }

    template <typename T, typename> bool all(T bitmask) { return ~bitmask == 0; }

    template <typename T, typename> bool contains(T mask_a, T mask_b) {
    ...
    template <typename T> xmtx4x4<T> matrix_rotation(const xquat<T>& rot_) {
        ...
        xmtx4x4<T> mtx_ = matrix_identity<T>();
    ...
    template <typename T> xmtx4x4<T> matrix_multiply(xmtx4x4<T> a, xmtx4x4<T> b) {
        xmtx4x4<T> mtx_ = {};
    ...
    template <typename T>
    xmtx4x4<T> compose_model_matrix(const xvec3<T>& position_, const xquat<T>& rotation_,
                                    const xvec3<T>& scale_) {
    ...
    template <typename T> xquat<T> to_quat(const xvec3<T>& euler_) {
        xquat<T> quat_ = {0};
    ...
    template <typename T> xvec3<T> to_euler(const xquat<T>& quat_) {
        xvec3<T> euler_{};
    ...
    template <typename T> xvec3<T> transform(const xvec3<T>& vec_, const xmtx4x4<T>& mtx_) {
        T w = mtx_[12] * vec_[0] + mtx_[13] * vec_[1] + mtx_[14] * vec_[2] + mtx_[15];
        return {(mtx_[0] * vec_[0] + mtx_[1] * vec_[1] + mtx_[2] * vec_[2] + mtx_[3]) / w,
                (mtx_[4] * vec_[0] + mtx_[5] * vec_[1] + mtx_[6] * vec_[2] + mtx_[7]) / w,
                (mtx_[8] * vec_[0] + mtx_[9] * vec_[1] + mtx_[10] * vec_[2] + mtx_[11]) / w};
    }

    template <typename T> T sq_magnitude(const xquat<T>& quat_) {
        return quat_[0] * quat_[0] + quat_[1] * quat_[1] + quat_[2] * quat_[2] +
               quat_[3] * quat_[3];
    }

    template <typename T> xquat<T> conjugate(const xquat<T>& quat_) {
        return {-quat_[0], -quat_[1], -quat_[2], quat_[3]};
    }

    template <typename T> xquat<T> inverse(const xquat<T>& quat_) {
        auto sq_mag = sq_magnitude(quat_);
        assert(sq_mag != 0);
        return conjugate(quat_) / sq_mag;
    }

    template <typename T> xmtx4x4<T> inverse(const xmtx4x4<T>& mtx_) {
    ...
    template <typename T> xvec3<T> negate(const xvec3<T>& v0) { return {-v0[0], -v0[1], -v0[2]}; }

    template <typename T> xvec3<T> multiply(const xvec3<T>& v0, float scalar) {
        return {
            v0[0] * scalar,
            v0[1] * scalar,
            v0[2] * scalar,
    ...
    template <typename T> xvec3<T> divide(const xvec3<T>& v0, float scalar) {
        return {
            v0[0] / scalar,
            v0[1] / scalar,
            v0[2] / scalar,
    ...
    template <typename T> xvec3<T> add(const xvec3<T>& v0, const xvec3<T>& v1) {
        return {
            v0[0] + v1[0],
            v0[1] + v1[1],
            v0[2] + v1[2],
    ...
    template <typename T> xvec3<T> subtract(const xvec3<T>& v0, const xvec3<T>& v1) {
        return {
            v0[0] - v1[0],
            v0[1] - v1[1],
            v0[2] - v1[2],
    ...
    template <typename T> T dot_product(const xvec3<T>& v0, const xvec3<T>& v1) {
        return v0[0] * v1[0] + v0[1] * v1[1] + v0[2] * v1[2];
    }

    template <typename T> xvec3<T> cross_product(const xvec3<T>& v0, const xvec3<T>& v1) {
        return {
            v0[1] * v1[2] - v0[2] * v1[1],
            v0[2] * v1[0] - v0[0] * v1[2],
            v0[0] * v1[1] - v0[1] * v1[0],
    ...
    template <typename T> T dist_sq(const xvec3<T>& a, const xvec3<T>& b) {
        return (b[0] - a[0]) * (b[0] - a[0]) + (b[1] - a[1]) * (b[1] - a[1]) +
               (b[2] - a[2]) * (b[2] - a[2]);
    }

    template <typename T> T dist(const xvec3<T>& a, const xvec3<T>& b) {
        return std::sqrt(dist_sq(a, b));
    }

    template <typename T> T magnitude_sq(const xvec3<T>& v) {
        return v[0] * v[0] + v[1] * v[1] + v[2] * v[2];
    }

    template <typename T> T magnitude(const xvec3<T>& v) { return std::sqrt(magnitude_sq(v)); }

    template <typename T> xvec3<T> lerp(const xvec3<T>& a, const xvec3<T>& b, float t) {
        return {
            a[0] + (b[0] - a[0]) * t,
            a[1] + (b[1] - a[1]) * t,
            a[2] + (b[2] - a[2]) * t,
    ...
    template <typename T> float invlerp(const xvec3<T>& a, const xvec3<T>& b, const xvec3<T>& v) {
        if (b[0] != a[0])
            return (v[0] - a[0]) / (b[0] - a[0]);
        else if (b[1] != a[1])
            return (v[1] - a[1]) / (b[1] - a[1]);
        else
